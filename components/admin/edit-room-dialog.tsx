...
export function EditRoomDialog({
  ...
  useEffect(() => {
    if (room) {
      reset({
        ...
        is_published: room.is_published ?? true,
  ...
  const onSubmit = async (data: RoomFormData) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/retreats/${retreatId}/rooms/${room.id}`, {
        method: 'PUT',
      ...
      toast.success(`Room "${data.name}" updated successfully`)
      onOpenChange(false)
      onSuccess()
    } catch {
  ...
  const handleEnableEarlyBird = (enabled: boolean) => {
    ...
    if (enabled && retreatStartDate && !watch('early_bird_deadline')) {
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-deposit_price">Deposit (EUR) *</Label>
              <Input
                id="edit-deposit_price"
                type="number"
                step="0.01"
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-available">Available Spots</Label>
              <Input
                id="edit-available"
                type="number"
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="edit-is_sold_out" className="text-sm">Mark as Sold Out</Label>
            </div>
            <div className="flex items-center space-x-2">
              <Switch
              ...
              />
              <Label htmlFor="edit-is_published" className="text-sm">Published</Label>
            </div>
          </div>
          ...
          <div className="bg-green-50 rounded-lg p-3 space-y-3">
            <div className="flex items-center gap-3">
              <Switch
                id="edit-early_bird_enabled"
                checked={earlyBirdEnabled}
                onCheckedChange={handleEnableEarlyBird}
              />
              <div className="flex-1">
                <Label htmlFor="edit-early_bird_enabled" className="text-sm font-medium text-green-800">
            ...
            {earlyBirdEnabled && (
              <div className="pt-3 border-t border-green-200">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <Label htmlFor="edit-early_bird_deadline" className="text-sm font-medium text-green-700">
                      Early Bird Deadline
                    </Label>
                    <Input
                      id="edit-early_bird_deadline"
                      type="date"
